...
#ifdef LOG_TAG
#undef LOG_TAG
#define LOG_TAG "im2d_rga_debugger"
#else
#define LOG_TAG "im2d_rga_debugger"
#endif
...
void rga_dump_channel_info(int log_level, const char *name, im_rect &rect, rga_buffer_t &image) {
    ...
    IM_LOG(log_level,
           ...
           "\timage[w,h,ws,hs,f] = [%d, %d, %d, %d, %s(0x%x)]\n"
           "\tbuffer[handle,fd,va,pa] = [0x%x, 0x%x, 0x%lx, 0x%lx]\n"
           "\tcolor_space[%s(0x%x)], global_alpha[0x%x], rd_mode[%s(0x%x)]\n",
           name,
           rect.x, rect.y, rect.width, rect.height,
           image.width, image.height, image.wstride, image.hstride, translate_format_str(image.format), image.format,
           ...
           image.global_alpha,
           string_rd_mode(image.rd_mode), image.rd_mode);
}
...
void rga_dump_opt(int log_level, im_opt_t &opt, int usage) {
    ...
    IM_LOG(log_level, "\tapi_version[0x%x]\n", opt.version);
